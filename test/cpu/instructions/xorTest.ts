...
describe('Instruction.XOR', () => {
    describe('when n is d8', () => {
        it('sets the right flags', () => {
            const rom = createRom([
                Instruction.toByte(Instruction.XOR('d8')),
                0x25, 
            ...
            assert.equal(cpu.registers.a, 0x34)
    ...
    describe('when n is B', () => {
        it('sets the right flags', () => {
            const rom = createRom([
                Instruction.toByte(Instruction.XOR('B')),
                Instruction.toByte(Instruction.Halt)
            ])
            const cpu = new CPU(undefined, rom)
            cpu.registers.a = 0x11
    ...
    describe('when n is (HL)', () => {
        it('sets the right flags', () => {
            const rom = createRom([
                Instruction.toByte(Instruction.XOR('(HL)')),
                Instruction.toByte(Instruction.Halt),
